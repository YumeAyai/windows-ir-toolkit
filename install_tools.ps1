...
.DESCRIPTION
    从 GitHub Releases 拉取最新的 tools-* release,自动放到 01_collection/ 和 02_analysis/
...
# 创建目录
New-Item -ItemType Directory -Force -Path "$OutputDir\01_collection\tools" | Out-Null
New-Item -ItemType Directory -Force -Path "$OutputDir\02_analysis\tools" | Out-Null
...
foreach ($asset in $release.assets) {
    ...
    if ($name -like "winpmem*") {
        $targetPath = Join-Path $OutputDir "01_collection\tools\$name"
...
if ($capaZip) {
    ...
    if ($capaExe) {
        ...
        $targetCapa = Join-Path $OutputDir "02_analysis\tools\capa"
        Copy-Item -Path $capaDir\* -Destination $targetCapa -Recurse -Force
        Write-Host "    ✓ capa 已放到 02_analysis\tools\capa\" -ForegroundColor Green
    }
}
...
if ($expectedHashes) {
    foreach ($file in Get-ChildItem $OutputDir\01_collection\tools, $OutputDir\02_analysis\tools\capa -Recurse -File -ErrorAction SilentlyContinue) {
...
Write-Host ""
Write-Host "[*] 接下来: 管理员运行 01_collection\run_collect.bat 开始取证" -ForegroundColor Cyan
